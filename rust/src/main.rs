...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Extract --palette <name> and --start-beat N if present.
    let mut palette_name: Option<String> = None;
    let mut start_beat: u32 = 0;
    let mut i = 0;
    while i < args.len() {
        if args[i] == "--palette" && i + 1 < args.len() {
            palette_name = Some(args.remove(i + 1));
            args.remove(i);
        } else if args[i] == "--start-beat" && i + 1 < args.len() {
            start_beat = args.remove(i + 1).parse().unwrap_or(0);
            args.remove(i);
        } else {
            i += 1;
        }
    ...
    let _host = AudioHost::start_with_palette(seed, palette_name.as_deref(), start_beat).map_err(
