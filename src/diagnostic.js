...
var DiagnosticPanel = {
  ...
  _buildDOM: function() {
    ...
    // Panel + glossary + toggle button
    root.innerHTML =
      '<style>' +
        '#diagPanel, #diagGlossary {' +
          'position:fixed; right:0; top:0; width:350px; height:100vh;' +
          'background:rgba(10,10,15,0.94); border-left:1px solid #1a1a3a;' +
          'font-family:"Courier New",monospace; font-size:12px; color:#aaaacc;' +
          'z-index:9999;' +
        '}' +
        '#diagPanel { display:none; flex-direction:column; overflow:hidden; }' +
        '#diagPanel.diag-visible { display:flex; }' +
        '#diagGlossary { display:none; z-index:10000; overflow-y:auto; padding:12px; background:rgba(10,10,15,0.97); }' +
        '#diagGlossary.diag-visible { display:block; }' +
        '#diagPanel button, #diagGlossary button {' +
          'background:none; border:1px solid #1a1a3a; color:#555577;' +
          'cursor:pointer; padding:4px 10px; margin-left:4px;' +
          'font-family:inherit; font-size:12px; min-width:32px; min-height:28px;' +
        '}' +
        '#diagPanel button:hover, #diagGlossary button:hover { background:#1a1a3a; color:#aaaacc; }' +
        '#diagToggle {' +
          'position:fixed; right:12px; top:12px;' +
          'min-width:44px; height:36px; padding:0 12px;' +
          'border-radius:18px; background:rgba(10,10,15,0.85);' +
          'border:1px solid #00ffcc66; color:#00ffcc;' +
          'font-family:"Courier New",monospace; font-size:11px; font-weight:bold;' +
          'cursor:pointer; z-index:9998; letter-spacing:1px;' +
          '-webkit-tap-highlight-color:transparent;' +
        '}' +
        '#diagToggle:hover { background:#00ffcc22; }' +
        '#diagToggle.diag-active { background:#00ffcc; color:#0a0a0f; border-color:#00ffcc; }' +
        '#diagToggle.diag-error { border-color:#ff3366; color:#ff3366; background:#ff336622; }' +
        '@media (max-width: 600px) {' +
          '#diagPanel, #diagGlossary { width:100vw; border-left:none; }' +
          '#diagPanel { font-size:13px; }' +
          '#diagPanel button, #diagGlossary button { min-height:40px; min-width:40px; padding:6px 12px; font-size:14px; }' +
          '#diagHeader { padding:14px 14px; }' +
          '#diagToggle { height:40px; min-width:52px; border-radius:20px; font-size:12px; }' +
        '}' +
      '</style>' +
      '<div id="diagPanel">' +
        ...
        'justify-content:space-between; align-items:center; flex-shrink:0;">' +
          ...
          '<span>' +
            '<button id="diagBtnGlossary">?</button>' +
            '<button id="diagBtnCopy">\uD83D\uDCCB</button>' +
            '<button id="diagBtnClose">\u00D7</button>' +
          '</span>' +
        '</div>' +
      ...
      '<div id="diagGlossary">' +
        '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">' +
          ...
          '<button id="diagGlossaryClose">\u00D7</button>' +
        '</div>' +
        '<div id="diagGlossaryBody"></div>' +
      '</div>' +
      '<button id="diagToggle" title="Toggle diagnostic panel (D)">DIAG</button>';
    ...
    // Toggle button (always visible — open/close panel, flashes on error)
    var toggle = document.getElementById('diagToggle');
    if (toggle) {
      toggle.onclick = function() { if (self._visible) self.hide(); else self.show(); };
    }
    this._badgeEl = toggle;
  },
  ...
  show: function() {
    ...
    if (panel) panel.classList.add('diag-visible');
    this._visible = true;
    this._hasUnseenError = false;
    if (this._badgeEl) {
      this._badgeEl.classList.add('diag-active');
      this._badgeEl.classList.remove('diag-error');
    }
    this._updateDisplay();
  },
  ...
  hide: function() {
    var panel = document.getElementById('diagPanel');
    if (panel) panel.classList.remove('diag-visible');
    this._visible = false;
    if (this._glossaryEl) this._glossaryEl.classList.remove('diag-visible');
    if (this._badgeEl) this._badgeEl.classList.remove('diag-active');
  },
  ...
  _flashError: function() {
    ...
    // Toggle button turns red when closed + unseen error
    if (!this._visible) {
      this._hasUnseenError = true;
      if (this._badgeEl) this._badgeEl.classList.add('diag-error');
    }
  },
  ...
  _toggleGlossary: function() {
    ...
    this._glossaryEl.classList.toggle('diag-visible');
  },
};
